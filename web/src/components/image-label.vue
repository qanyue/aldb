...
const loadPictureAndInitCanvas = () => {
  ...
  img.id = "img";
  img.src = url;
  console.log("img.src in label.vue: " + img.src)
  // image loaded
  img.onload = function () {
    loading.value = false;
    ...
    if (canvas.value) {
      ...
      console.log("canvans width in label.vue " + canvas.value.style.width)
...
const mouseWheelDrawPicture = (e: MouseEvent) => {
  ...
  if (deltaY < 0 && getScale() < 5 && canvas?.value) {
    ...
    // canvas.value.width = containerW.value;
    // canvas.value.height = containerH.value;
  }
...
<template>
  <div ref="container" class="image-label__container" :style="{
    width: containerW + 'px',
